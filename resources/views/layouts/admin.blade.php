...
<head>
    ...
    <style>
      /* simple admin color to differ from user */
...
<body>
    <nav class="topnav">
        ...
        @if(Route::has('admin.edu.index'))
          <a href="{{ route('admin.edu.index') }}">Edukasi</a>
        @endif
    ...
    <div class="container">
        @if(session('success')) <div style="background:#e9f9ef;padding:10px;border-left:4px solid #0b5d39;margin-bottom:12px;">{{ session('success') }}</div> @endif
        @if(session('error')) <div style="background:#fff0f0;padding:10px;border-left:4px solid #c0392b;margin-bottom:12px;">{{ session('error') }}</div> @endif
